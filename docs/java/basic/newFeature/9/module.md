...
在模块化之前，只要你拿到了 Class 对象，通过 setAccessible(true) 就可以强行访问任何私有成员。
但在模块化下，即便代码在同一个进程里，如果一个包没有被显式 opens，反射也会失败

exports com.jasper.api;：别人可以调用你的代码，但不能通过反射访问私有字段。
opens com.jasper.api;：别人可以通过反射访问该包下的所有成员（包括私有）
