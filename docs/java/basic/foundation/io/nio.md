...
- [directory memory](/docs/java/basic/jvm/runtimeDataArea/directMemory.md)

DirectByteBuffer，java.nio.ByteBuffer 类提供的静态方法 allocateDirect(int capacity) 来实现。

与普通的 allocate(int capacity)（在 JVM 堆上分配）不同，这个方法直接在操作系统的本地内存中分配空间。
...
- 数据量极小： 比如只有几百字节，堆内存更划算。
- 频繁销毁： 如果不打算做 Buffer 池，频繁分配直接内存会导致严重的性能下降。
- 纯逻辑处理： 如果数据只是在 Java 内部进行复杂的算法计算，不涉及网络或磁盘 I/O，那么留在堆里更利于 GC 优化
